...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (rowId === undefined || rowId === null) {
      return NextResponse.json(
        { success: false, error: 'rowId diperlukan' },
        { status: 400 }
      );
    }
    
    await deleteKejadianRow(rowId);
    return NextResponse.json({ success: true, isDemo: false });
  } catch (error) {
    console.error('DELETE Error:', error);
    const errorMessage = error instanceof Error ? error.message : 'Gagal menghapus data dari Google Sheets';
    return NextResponse.json(
